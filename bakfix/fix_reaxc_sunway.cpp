...
FixReaxCSunway::FixReaxCSunway(LAMMPS *lmp,int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
double FixReaxCSunway::memory_usage()
{
  int nmax = atom->nmax;
...
void FixReaxCSunway::grow_arrays(int nmax)
{
...
void FixReaxCSunway::copy_arrays(int i, int j, int delflag)
{
...
int FixReaxCSunway::pack_exchange(int i, double *buf)
{
  buf[0] = num_bonds[i];
  buf[1] = num_hbonds[i];
  return 2;
...
int FixReaxCSunway::unpack_exchange(int nlocal, double *buf)
{
...
int FixReaxCSunway::pack_forward_comm(int n, int *list, double *buf,
...
void FixReaxCSunway::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
