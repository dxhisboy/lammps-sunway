...
#include "fix_reaxc_species_sunway.h"
#include "domain.h"
#include "update.h"
...
FixReaxCSpeciesSunway::FixReaxCSpeciesSunway(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
FixReaxCSpeciesSunway::~FixReaxCSpeciesSunway()
{
  memory->destroy(ele);
...
int FixReaxCSpeciesSunway::setmask()
{
  int mask = 0;
  mask |= POST_INTEGRATE;
  return mask;
...
void FixReaxCSpeciesSunway::setup(int vflag)
{
...
void FixReaxCSpeciesSunway::init()
{
...
void FixReaxCSpeciesSunway::create_compute()
{
  int narg;
  char **args;
...
void FixReaxCSpeciesSunway::create_fix()
{
  int narg;
  char **args;
...
void FixReaxCSpeciesSunway::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixReaxCSpeciesSunway::post_integrate()
{
...
void FixReaxCSpeciesSunway::Output_ReaxC_Bonds(bigint ntimestep, FILE *fp)
...
AtomCoord FixReaxCSpeciesSunway::chAnchor(AtomCoord in1, AtomCoord in2)
{
  if (in1.x < in2.x)
    return in1;
  else if (in1.x == in2.x) {
...
void FixReaxCSpeciesSunway::FindMolecule ()
{
...
void FixReaxCSpeciesSunway::SortMolecule(int &Nmole)
{
...
void FixReaxCSpeciesSunway::FindSpecies(int Nmole, int &Nspec)
{
...
int FixReaxCSpeciesSunway::CheckExistence(int id, int ntypes)
{
  int i, j, molid, flag;
...
void FixReaxCSpeciesSunway::WriteFormulas(int Nmole, int Nspec)
{
  int i, j, itemp;
...
void FixReaxCSpeciesSunway::OpenPos()
{
  char *filecurrent;
  bigint ntimestep = update->ntimestep;
...
void FixReaxCSpeciesSunway::WritePos(int Nmole, int Nspec)
{
  int i, itype, cid;
...
double FixReaxCSpeciesSunway::compute_vector(int n)
{
  if (n == 0)
    return vector_nmole;
  if (n == 1)
...
int FixReaxCSpeciesSunway::nint(const double &r)
{
  int i = 0;
...
int FixReaxCSpeciesSunway::pack_forward_comm(int n, int *list, double *buf,
...
void FixReaxCSpeciesSunway::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
double FixReaxCSpeciesSunway::memory_usage()
{
  double bytes;
